...
test("simple rendering", async () => {
    ...
    expect(".o_app[data-menu-xmlid=app_1] .o_app_icon").toHaveAttribute("data-src", "/web/static/img/default_icon_app.png");
    ...
    expect(".o_app[data-menu-xmlid=app_2] .o_app_icon").toHaveStyle({ backgroundColor: "rgb(198, 87, 42)" });
    expect(".o_app[data-menu-xmlid=app_2] i.fa.fa-diamond").toHaveStyle({ color: "rgb(255, 255, 255)" });
    expect(".o_app[data-menu-xmlid=app_2] .o_web_studio_edit_icon i").toHaveCount(1);

    expect(".o_web_studio_new_app .o_app_icon").toHaveAttribute("data-src", "/web_studio/static/src/img/default_icon_app.png");
...
test("Click on edit icon button", async () => {
    ...
    await contains(".o_app[data-menu-xmlid=app_1] .o_web_studio_edit_icon", { visible: false }).click();

    expect(".o_dialog").toHaveCount(1);
    expect(".o_dialog header.modal-header").toHaveText("Edit Application Icon");
    expect(".o_dialog .modal-content.o_web_studio_edit_menu_icon_modal .o_web_studio_icon_creator").toHaveCount(1);
...
test("edit an icon", async () => {
    onRpc("/web_studio/edit_menu_icon", async (request) => {
        ...
        MockServer.current.menus[1].webIcon = params.icon.join(",");
        return true;
    });
